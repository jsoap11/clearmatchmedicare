...
class Common_model extends CI_Model
{
	...
	function __construct()
	{
		...
		public function common_blogs($limit, $start) 
	{
		$this->db->distinct();
		...
		public function common_blog($limit, $start,$txt_search) 
	{
		$this->db->distinct();
		$this->db->select("blog_post.*,users.first_name,users.last_name,blog_categories.cat_name,blog_categories.slug AS cat_slug");
		$this->db->from("blog_post");
		...
		public function common_blog_count($txt_search) 
	{
		$this->db->distinct();
		$this->db->select("blog_post.*,users.first_name,users.last_name,blog_categories.cat_name,blog_categories.slug AS cat_slug");
		$this->db->from("blog_post");
		...
		public function getSearchData($getSearchData) 
	{
		$this->db->distinct();
		...
		$this->db->like('title', $getSearchData);
		$this->db->order_by("blog_post.id","desc");
		$this->db->where('blog_post.deleted_at ', "Null");
		$this->db->where('blog_post.status ', 1);
		$this->db->where('blog_post.type ', 0);
	...
	function getAllDataSearch($table,$specific='',$row='',$Where='',$order='',$limit='',$groupBy='',$like = '')
	{
		// If Condition
		if (!empty($Where)):
		$this->db->where($Where);
