...
class Auth extends MY_Controller
{
	...
	public function login()
	{
		...
		if ($this->form_validation->run() == true) {
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember)) {
				...
				if ($check[0]->two_factor_auth == 1) {
					...
					if ($send) {
						...
						redirect('backoffice/Auth/authentication', 'refresh');
					} else {
				...
				redirect('backoffice/auth/login', 'refresh'); // use redirects instead of loading views for compatibility with MY_Controller libraries
			}
		} else {
			...
			$this->session->set_flashdata('message', $this->ion_auth->messages());
			$this->load->view('backoffice/auth/login', $data);
		}
	}
	...
	*/
	public function Authentication()
	{
		if ($this->input->post()) {
			...
			view("backoffice/auth/two_factor_auth");
		}
	}
	...
	public function forgot_password()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
			$this->_render_page('backoffice/auth/forgot_password', $data);
		} else {
			...
			if (empty($identity)) {
				...
				$this->session->set_flashdata('message', $this->ion_auth->errors());
				redirect("backoffice/auth/forgot_password", 'refresh');
			}
			...
			if ($forgotten) {
				...
				$this->session->set_flashdata('message', $this->ion_auth->messages());
				redirect("backoffice/auth/login", 'refresh'); //we should display a confirmation page here instead of the login page
			} else {
				$this->session->set_flashdata('message', $this->ion_auth->errors());
				redirect("backoffice/auth/forgot_password", 'refresh');
			}
	...
	public function deactivate($id = NULL)
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data['page'] = 'backoffice/auth/deactivate_user';
