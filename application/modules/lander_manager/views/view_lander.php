                     ...
                     <tbody>
                        <?php foreach ($lander as $rx_part) : ?>
                           <tr>
							   ...
							   <td>
								   <a href="<?= base_url('lander_manager/edit') ?>/<?=  $rx_part->id ?>">
									   <button class="btn btn-rounded btn-space btn-primary"><span class="icon mdi mdi-edit"></span> Edit</button>
								   </a>
								   <?php if($rx_part->id !=2){?>
								   <button onclick="deleteFunction('<?php echo base_url('lander_manager/delete_lander').'/'.  $rx_part->id; ?>')" class="btn btn-rounded btn-space btn-danger" disable>
